...
async function getPrivateS3Json(key: string): Promise<any> {
  try {
    // --- CÓDIGO SIMPLIFICADO ---
    // Al no especificar 'accessLevel', Amplify usará directamente los permisos
    // del rol IAM, que ya apuntan a las carpetas correctas.
    const getUrlResult = await getUrl({ key: key });
